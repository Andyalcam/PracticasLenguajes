...
(define (check-type fwael-expr)
    (cond
        [(num? fwael-expr) numV]
        [(bool? fwael-expr) boolV]
        [(op? fwael-expr)
                (cond
                    [(list? (member (op-f fwael-expr) (list + - * / modulo exp))) numV]
                    [(list? (member (op-f fwael-expr) (list not = < > <= >=))) boolV]
                    [else ("error")])
            ]
        [(op-bool? fwael-expr) boolV]
        [(branch? fwael-expr) 'TCFWAEL-Value]
        [(fun? fwael-expr) (check-type (fun-body fwael-expr))]
        [(lempty? fwael-expr) listV]
        [(lcons? fwael-expr) listV]
        [(lcar? fwael-expr) 'TCFWAEL-Value]
        [(lcdr? fwael-expr) listV]
        [(app? fwael-expr) (check-type (app-fun fwael-expr))]
        [else (error "ERROR" (fwael-expr) "no es correcta")]
    )
)